...
class OfficerServiceTest {
    ...
    void setUp() throws IOException {
        ...
        officer = new Officer(null, "officer1", "pw", "o@ntu.sg", Role.OFFICER, "S1111111A", 30, MaritalStatus.SINGLE,
                null, null, null, null);
    ...
    void registerForProjectAndViewRegistration() throws IOException {
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "Proj1", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Tampines", 100, true, 5, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 3)));
    ...
    void getRegistrableProjects_filtersCorrectly() throws IOException {
        Project p1 = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "P1", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Yishun", 100, true, 5, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 2)));
    ...
    void getHandlingEnquiries_returnsCorrectly() throws IOException {
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "EProj", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Bedok", 100, true, 2, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 1)));
        officer.setProjectId(project.getId());
        officer = (Officer) RepositoryDependency.getUserRepository().save(officer);

        Enquiry e = RepositoryDependency.getEnquiryRepository()
                .save(new Enquiry(null, "Any left?", project.getId(), 321));
    ...
    void replyEnquiry_success() throws IOException {
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "ReplyProj", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Bishan", 100, true, 3, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 2)));
        officer.setProjectId(project.getId());
        officer = (Officer) RepositoryDependency.getUserRepository().save(officer);

        Enquiry e = RepositoryDependency.getEnquiryRepository()
                .save(new Enquiry(null, "Can I apply?", project.getId(), 123));
    ...
    void bookFlatAndGenerateReceipt_success() throws IOException {
        ...
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "BookingProj", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Woodlands", 100, true, 1, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 1)));

        Application app = RepositoryDependency.getApplicationRepository().save(new Application(null, applicant.getId(),
                project.getId(), ApplicationStatus.SUCCESSFUL, false, FlatType._2ROOM));
    ...
    void getHandlingApplications_returnsCorrectApps() throws IOException {
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "AppProj", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Clementi", 100, true, 3, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._3ROOM, 2)));
        officer.setProjectId(project.getId());
        RepositoryDependency.getUserRepository().save(officer);

        Applicant a1 = new Applicant(null, "App1", "pw", "1@a.com", Role.APPLICANT, "S0000001A", 35,
                MaritalStatus.SINGLE, null, null);
        a1 = (Applicant) RepositoryDependency.getUserRepository().save(a1);
        applicationService.save(
                new Application(null, a1.getId(), project.getId(), ApplicationStatus.BOOKED, false, FlatType._3ROOM));
    ...
    void bookFlatAfterProjectClosed_shouldStillSucceed() throws IOException {
        ...
        Project project = RepositoryDependency.getProjectRepository().save(new Project(null, "ClosedProj", open, close,
                "Jurong", 200, false, 2, new HashSet<>(), new HashSet<>(), Map.of(FlatType._3ROOM, 1)));
        ...
        Application app = RepositoryDependency.getApplicationRepository().save(new Application(null, applicant.getId(),
                project.getId(), ApplicationStatus.SUCCESSFUL, false, FlatType._3ROOM));
        applicant.setApplicationId(app.getId());
        RepositoryDependency.getUserRepository().save(applicant);
        ...
        Applicant updatedApplicant = (Applicant) RepositoryDependency.getUserRepository()
                .findByNric(applicant.getNric());
    ...
    void testOfficerAccessAfterProjectDeleted_returnsNullOrEmptyGracefully() throws IOException {
        // Create project
        Project project = RepositoryDependency.getProjectRepository()
                .save(new Project(null, "TempProj", new Date(System.currentTimeMillis() - 86400000L),
                        new Date(System.currentTimeMillis() + 86400000L), "Jurong", 200, true, 2, new HashSet<>(),
                        new HashSet<>(), Map.of(FlatType._2ROOM, 2)));

        // Create applicant and application
        final Applicant applicant = (Applicant) RepositoryDependency.getUserRepository().save(new Applicant(null, "App",
                "pw", "app@ntu.sg", Role.APPLICANT, "S0000123X", 34, MaritalStatus.SINGLE, null, null));
        ...
        Enquiry enquiry = RepositoryDependency.getEnquiryRepository()
                .save(new Enquiry(null, "Is this project available?", project.getId(), applicant.getId()));

        // Create officer and registration
        final Officer officer = (Officer) RepositoryDependency.getUserRepository().save(new Officer(null, "O", "pw",
                "officer@hdb.sg", Role.OFFICER, "S1111111A", 30, MaritalStatus.SINGLE, null, null, null, null));
        Registration reg = RepositoryDependency.getRegistrationRepository()
                .save(new Registration(null, officer.getId(), project.getId(), RegistrationStatus.APPROVED));
        officer.setProjectId(project.getId());
